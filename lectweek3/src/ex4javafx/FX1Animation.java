...
/*
    Basic JavaFX Animation
    Just for the curious, will not show up on exam!
    All graphics implemented in labs

    NOTE: There are many more ways (more advanced)
 */

public class FX1Animation extends Application {
    ...
    void update(long now) {
        if (now - lastUpdateTime > 100_000_000) {
            x = x + 1;        // Move it
            lastUpdateTime = now;
        }
    }
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        GraphicsContext gc = canvas.getGraphicsContext2D();  // Get the toolbox for this drawing area
